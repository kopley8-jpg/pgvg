...
export const CompatiblePlatsEntry = (props: ICompatiblePlatsEntry) => {
  const { compatiblePlats, objEntryStyles, onDelete, onPlatItemClick, onPlatAdd } = props
  return (
    <ObjEntryTwo
      ...
      renderForKeys={[
        ...compatiblePlats.map(plat =>
          createRenderConfig({ plat }).forKeys(["plat"], (_key, value) => (
            <div style={{ display: "flex", flexDirection: "row" }}>
              ...
              <IconButton size="small" onClick={() => {
                onDelete(compatiblePlats.filter(res => res.idFromPlatforms != plat.idFromPlatforms))
              }}>
        ...
        ...createRenderConfig({ a: 1 }).forKeys(["a"], () => (
          <IconButton size="small" onClick={e => { onPlatAdd(e) }}>
            <Add fontSize="small" />
          </IconButton>
        ), { hideKeyName: true })
