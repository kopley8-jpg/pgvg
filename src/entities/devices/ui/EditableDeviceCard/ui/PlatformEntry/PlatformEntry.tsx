...
import { PLATFORM_FORMATS, type CompactiblePlatType, type PlatformType } from '@/shared/types/device';
...
interface IPlatformEntry {
  ...
  onCompatiblePlatAdd: (e: React.MouseEvent<HTMLButtonElement, MouseEvent>) => void
}

export const PlatformEntry = (props: IPlatformEntry) => {
  const { platform, objEntryStyles, onChange, onPlatItemClick, onCompatiblePlatAdd } = props;
      ...
      renderForKeys={[
        ...createRenderConfig(platform).forKeys(['type'], (_key, value) => (
          <DropDownList
            ...
            onPick={(picked) => {
              ...
              if (picked === "магнит") {
                onChange({ type: picked, adjustmentAirflow: false, compatiblePlats: [] })
              } else {
        ...
        ...(
          platform.type === "магнит"
            ? [
              ...createRenderConfig(platform).forKeys(["adjustmentAirflow"], (_key, value) => (
                <Switch checked={value} size='small' onClick={() => onChange({ ...platform, adjustmentAirflow: !platform.adjustmentAirflow })} />
              )),
              ...createRenderConfig(platform).forKeys(["compatiblePlats"], (_key, value) => (
