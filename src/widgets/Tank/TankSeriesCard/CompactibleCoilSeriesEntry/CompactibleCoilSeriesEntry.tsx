...
export const CompactibleCoilSeriesesEntry = (
  props: ICompactibleCoilSeriesesEntry
) => {
  const { compatibleCoilSerieses } = props;
  const { clickedCoilSeriesId, handler } =
    useCompatibleCoilSeriesesEntry(props);
  return (
    <ObjEntryTwo
      ...
      renderForKeys={[
        ...compatibleCoilSerieses
          .map((series) => [
            ...createRenderConfig({ series }).forKeys(
              ['series'],
              (key, value) => (
                <>
                  <PopupState variant="popover">
                    {(state) => (
                      <>
                        <CoilItem
                          compatibleCoilSeries={value}
                          {...handler.coilItem}
                          onClick={() => {
                            handler.coilItem.onClick(value.idFromPlatforms);
                            state.open();
                          }}
                        />
                        <CoilSeriesCardModal
                          {...bindMenu(state)}
                          open={state.isOpen}
                          coilSeriesId={value.idFromPlatforms}
                        />
                      </>
                    )}
                  </PopupState>
