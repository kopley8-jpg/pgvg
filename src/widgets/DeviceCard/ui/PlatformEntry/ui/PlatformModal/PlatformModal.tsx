...
export const PlatformModal = ({ plat, isOpen, onBackdropClick }: IPlatformModal) => {
  ...
  return (
    <Modal sx={styles.modal} open={isOpen} onClose={onBackdropClick}>
      {platform.type === "pod" ? (
        <PodSeriesCard platform={platform} />
      ) : (
        <></>
      )}
    </Modal>
...
const usePlatform = (
  ...
  const [platform, setPlatform] = useState<{ plat: PodSeriesType, type: "pod" } | { plat: TankSeriesType, type: "tank" } | null>(null)
  const platRef = ref(database, `/kochegar/platform/${plat.type === "pod" ? "pods" : "tanks"}/${plat.id}`)
