    ...
    <>
      {battery ? (
          ...
          renderForKeys={[
            ...createRenderConfig(battery).forKeys(['type'], (_key, value) => (
              <DropDownList
                ...
                onPick={handleDropDownPicked}
              />
            )),
            ...(battery.type === 'встроенный'
              ? createRenderConfig(battery).forKeys(
                  ['capacity'],
                  (_key, value) => (
                    <TextValue
                      fontSize={'5vw'}
                      ...
                      errorOptions={{
                        errorText: 'емкость?',
                        onErrorTextClick: handleCapacityErrorTextClick,
                      }}
                      onSaveButtonPress={handleCapacityChanged}
                    />
                  )
                )
        ...
        <span
          ...
          onClick={() => onChange({ type: 'сменный', format: null })}
        >
          АКБ?
        </span>
      )}
